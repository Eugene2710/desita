...
import {device_dpi} from '../data.js'

function Cruler() {
  let dpi = 0;

  // const options = ["iPhone", "iPad", "Samsung", "Others"]

  const [selected, setSelected] = useState(device_dpi[0].reported_dpi)
  ...
  //       return mid;
  //     }
  //     return binSearch(fn, min, mixd - 1);
  //   }
  ...
  for (let i=100; i<=3000; i+=100) {
    let pix = i*0.0000393701*selected
  ...
  return (
    ...
    <form> 
      ...
      onChange={(e) => setSelected(e.target.value)}>
        {device_dpi.map(object => (
          <option key={`dropdown_option_${object.name}`} value={object.reported_dpi}>
            {object.name}
          </option>
        ))}
      </select>
